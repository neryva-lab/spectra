...
Unless stated otherwise, main-paper tables aggregate over seeds 42, 43, and 44. The synthetic scale-stress table uses 10 seeds, as stated in its caption. The stress studies use per-seed \texttt{results.csv} summaries. NYUv2, Yeast, and RF1 tables are extracted from epoch-level \texttt{metrics.csv} logs using the final recorded epoch for each run. This final-epoch rule is especially important for NYUv2 because checkpoint metadata is not consistently available across methods.
