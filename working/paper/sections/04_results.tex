...
\input{data/stress/rescaling/tables/scale_stress_rescaling_table}
...
Table~\ref{tab:nyuv2_main} shows that no method dominates every metric. Static achieves the best
segmentation mIoU, and UWSO is strongest on both surface-normal metrics. BPGS gives the strongest
depth results and aggregate trade-off: best absolute relative error (0.223), depth RMSE (0.790),
total loss (1.891), and $\Delta_M$ (0.078). Appendix Figures~\ref{fig:appendix_nyuv2_bars}--\ref{fig:appendix_nyuv2_delta} show the same trade-off.

\input{data/nyuv2/tables/nyuv2_main_table}
...
Nash-MTL has the weakest mean mIoU (0.252) and largest mIoU standard deviation (0.027), while its
within-$11.25^\circ$ accuracy (0.211) exceeds Static (0.169). This supplies a recent
multi-objective baseline under the same NYUv2 protocol without establishing a general ranking.

\subsection{Ablation on chart and initialization}
Table~\ref{tab:ablation} isolates the batch-aware chart from initialization. The stateless
auto-calibrated variant degrades severely (mIoU 0.058; total loss 6.142), indicating that
first-batch calibration alone is insufficient for stable behavior. Appendix
Figure~\ref{fig:appendix_ablation_val_curves} shows the failure in validation dynamics, and
Figure~\ref{fig:appendix_ablation_weights} shows the contrasting task-weight behavior.

\input{data/ablation/tables/ablation_table}
...
task metrics have coefficients of variation below 2\%, and the final $\theta_{\max}$ values differ by
...
\input{data/full_data/tables/real_data_table}
