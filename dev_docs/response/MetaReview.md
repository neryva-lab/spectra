We have addressed the following suggestion.

## 1. Design choices not sufficiently justified before
...
- **Batch-aware conditioning.** Centring on the batch log-loss mean and scaling by the batch spread makes the coordinate relative to the current batch. This gives rescaling invariance (see MR-9).
...
## 2. Relationship to existing techniques not explained; positioning relative to prior work limited
...
## 3. Choice in Eq. (4) not explained
...
## 4. Empirical evaluation narrow; results modest; problem motivation and significance not established strongly enough
...
## 5. Novelty not established strongly enough
